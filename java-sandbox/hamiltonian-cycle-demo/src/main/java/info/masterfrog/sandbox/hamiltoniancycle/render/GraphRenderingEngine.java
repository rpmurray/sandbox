...
public class GraphRenderingEngine implements RenderingEngine {
    ...
    private void drawEdges(Graphics2D graphics2D) {
        ...
        for (Pair<Integer> e : graph.getEdges()) {
            ...
            int xDiff = p1.x - p2.x;
            int yDiff = p1.y - p2.y;
            double slope = xDiff == 0 ? -1.0 : (yDiff / (double) xDiff);
            double length = Math.sqrt(Math.pow(xDiff, 2) + Math.pow(yDiff, 2));
            int paddingScale = (int) (vertexDiameter);
            int x1 = xDiff == 0 ? p1.x : p1.x + (int) Math.floor(paddingScale);
            int y1 = p1.y + (int) Math.floor(paddingScale);
            int x2 = xDiff == 0 ? p2.x : p2.x - (int) Math.floor(paddingScale);
            int y2 = p2.y - (int) Math.floor(paddingScale);
